...
export async function POST(request: NextRequest) {
  try {
    ...
    if (nameResult.error) {
      console.error('Top-up search name error:', nameResult.error)
      return NextResponse.json({ error: nameResult.error.message }, { status: 500 })
    }
    if (emailResult.error) {
      console.error('Top-up search email error:', emailResult.error)
      return NextResponse.json({ error: emailResult.error.message }, { status: 500 })
    }
    if (phoneResult.error) {
      console.error('Top-up search phone error:', phoneResult.error)
    ...
    if (walletsError) {
      console.error('Top-up search wallets error:', walletsError)
    ...
    if (missingUserIds.length > 0) {
      ...
      if (insertError) {
        console.error('Top-up search wallet insert error:', insertError)
    ...
    console.error('Top-up search route error:', error)
