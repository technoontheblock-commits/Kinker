...
export default function AdminBonusCardDetailPage({ params }: { params: { id: string } }) {
  ...
  async function loadCard() {
    try {
      const response = await fetch(`/api/bonuscard/${params.id}`)
  ...
  async function loadScans() {
    try {
      const response = await fetch(`/api/bonuscard/scan-history?card_id=${params.id}`)
      const data = await response.json()
      if (data.scans) {
        setScans(data.scans)
      }
  ...
  async function updateStatus(updates: { status?: string; payment_status?: string }) {
    try {
      const response = await fetch(`/api/bonuscard/admin/${params.id}/status`, {
        method: 'PATCH',
